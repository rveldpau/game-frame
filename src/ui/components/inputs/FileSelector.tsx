...
export function FileSelector({value, onChange, title, filters, defaultPath, openDirectory}: FileSelectorProps){
    const api = React.useContext(APIContext);
    const selectFile = React.useCallback(() => {
        api.dialogs.selectFileForOpen({
            title: title ?? (openDirectory ? "Select Directory":"Select file"),
            filters,
